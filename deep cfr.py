...
def main(iter, trav):
    ...
    for t in range(iter):
        ...
        p_not = (p+1)%2
        for n in range(trav):
            collect_samples(node(), p, p_not M_r, B_v, B_s, t)
        M_r[p].train(B_v)
...
def collect_samples(h, p, p_not, M_r, B_v, B_s, t):
    if h.is_terminal():
        return h.util(p)
    elif h.P() == p:
        I = h.I(p)
        ...
        v_a = np.zeros(3)
        for a in h.A():
            v_a[a] = collect_samples(h.place(a), p, p_not, M_r, B_v, B_s, t)
        v_s = np.dot(v_a, sigma)
        d = v_a - v_s
        B_v[p].append((I, d, t)) # change
    elif h.P() == p_not:
        sigma = calculate_strategy(I, M_r[p_not])
        B_s.append((h.I(p_not), sigma, t))
        a = np.random.choice(h.A(), p=sigma)
        return collect_samples(h.place(a), p, p_not, M_r, B_v, B_s, t)
    else:
